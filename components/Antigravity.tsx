...
//     const m = mouseRef.current.active
//       ? {
//           x: (mouseRef.current.x / size.width) * 2 - 1,
//           y: -(mouseRef.current.y / size.height) * 2 + 1,
//         }
//       : state.pointer;
...
const AntigravityInner: React.FC<AntigravityProps> = ({
  ...
  waveSpeed = 0.4,
  waveAmplitude = 1,
  particleSize = 0.15,        // ← reduced from 0.4
  lerpSpeed = 0.05,
  color = '#FF9FFC',
  autoAnimate = true,
  particleVariance = 1,
  ...
  useFrame(state => {
    ...
    particles.forEach((particle, i) => {
      ...
      // ← particleSize now directly controls final size; no hidden floor
      const finalScale = scaleFactor * (0.8 + Math.sin(t * pulseSpeed) * 0.2 * particleVariance) * particleSize;
      dummy.scale.set(finalScale, finalScale, finalScale);
  ...
  return (
    <instancedMesh ref={meshRef} args={[undefined, undefined, count]}>
      {/* ↓ All geometry args halved so particleSize prop has real effect */}
