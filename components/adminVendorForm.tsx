...
export default function AdminVendorForm({ open, setOpen, onSuccess, editData }) {
  const [vendors, setVendors] = useState([])
  const [form, setForm] = useState({ vendorId: "", amount: "", description: "" })
  ...
  const fetchVendors = async () => {
    try {
      ...
      const list = (all || []).filter((u) => String(u.role || '').toLowerCase() === 'vendor')
      setVendors(list)
    } catch (err) {
  ...
  useEffect(() => {
    ...
    if (open && editData) {
      setForm({
        vendorId: editData.vendor?._id || editData.vendor || "",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(url, {
        ...
        body: JSON.stringify({
          title: editData?.title || `Vendor payment`,
          description: form.description || 'Vendor payment',
          amount,
          totalFee: amount,
      ...
      notify.success(editData ? 'Payment updated' : 'Payment recorded', editData ? 'Vendor payment updated successfully' : 'Vendor payment recorded successfully')
      setOpen(false)
      onSuccess && onSuccess()
    } catch (err) {
      ...
      <div className="w-full max-w-md rounded-2xl shadow-2xl overflow-visible bg-white dark:bg-zinc-900 border border-gray-200 dark:border-white/10">
        <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100 dark:border-white/10">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-sm font-bold text-gray-900 dark:text-white">Add Vendor Payment</h3>
              <p className="text-xs text-gray-400 dark:text-gray-500">Assign payment to a vendor</p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          <div>
            <label className="block text-xs font-semibold uppercase tracking-wider mb-1.5 text-gray-500 dark:text-gray-400">Vendor</label>
            <div>
              <select
                value={form.vendorId}
              ...
              >
                ...
                {vendors.map((v) => (
                  <option key={v._id} value={v._id} className="dark:bg-zinc-800 dark:text-white">
