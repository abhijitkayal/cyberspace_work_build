...
//     background: `radial-gradient(00px at ${coords.x}px ${coords.y}px, rgba(0,150,255,0.2), transparent 60%)`,
...
const WhyChooseUsWithGraph = () => {
  ...
  // CardWrapper: removed the border-based gradient wrapper that caused the ghost line.
  // Now uses a subtle box-shadow + background instead.
  const CardWrapper = ({ children, className = "" }) => (
    <div
      className={`relative rounded-2xl overflow-hidden ${className}`}
      style={{
        background: "rgba(255,255,255,0.04)",
        boxShadow: "0 0 0 1px rgba(0,200,255,0.12), 0 4px 24px rgba(0,0,0,0.3)",
      }}
    >
      <div className="relative bg-transparent rounded-2xl p-6 flex flex-col gap-4 h-full transition-all duration-500">
        {children}
      </div>
    </div>
  );
  ...
  return (
    <main className="relative" style={{ background: "#000000" }}>
      ...
      <div
        ...
        style={{
          background:
            "radial-gradient(circle at 50% 50%, rgba(14,186,199,0.18), transparent 30%)",
        }}
      />
      ...
      <section className="pt-2 relative z-10 px-6">
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-6">
            ...
            >
              ...
              <p className="text-gray-300 mb-8 text-center lg:text-left">
                At Cyberspace Works, we go beyond just delivering digital solutions — we build
                experiences that drive growth. Our team of passionate developers, designers, and
                strategists work hand in hand to craft innovative, user-focused, and result-driven
                products. From concept to launch, we ensure every project reflects precision,
                creativity, and measurable impact. With transparent communication, on-time delivery,
                and cutting-edge technology, we're the digital partner you can rely on to bring your
                vision to life.
              </p>
            ...
            {/* ── RIGHT (CHART) ── */}
            {/*
              KEY FIX: Replaced `border border-white/10` with a box-shadow ring.
              The old `border` rendered a 1px solid line on all 4 sides — the left edge
              of this div fell exactly on the grid midpoint, creating the visible
              vertical "ghost line" seen in the screenshot.
              box-shadow: inset 0 0 0 1px ... keeps the visual ring but is rendered
              entirely INSIDE the element, so it never bleeds into adjacent columns.
            */}
            <motion.div
              ref={rightRef}
              initial={{ opacity: 0, x: 120 }}
              animate={rightInView ? { opacity: 1, x: 0 } : {}}
              transition={{ duration: 0.6, ease: "easeOut" }}
              className="relative rounded-2xl p-8 bg-black/60 backdrop-blur-xl overflow-hidden shadow-lg group transition-all duration-300"
              style={{
                boxShadow:
                  "inset 0 0 0 1px rgba(255,255,255,0.08), 0 0 0 0 transparent",
              }}
              onMouseEnter={(e) => {
                e.currentTarget.style.boxShadow =
                  "inset 0 0 0 1px rgba(255,255,255,0.08), 0 0 18px rgba(0,150,255,0.35)";
              }}
              onMouseLeave={(e) => {
                e.currentTarget.style.boxShadow =
                  "inset 0 0 0 1px rgba(255,255,255,0.08), 0 0 0 0 transparent";
            ...
            >
              ...
              <p className="text-gray-400 text-sm mt-8 leading-relaxed">
                Clients come to us because we turn ideas into impactful digital
                experiences — fast, reliable, and beautifully built.
              </p>
            </motion.div>
          </div>
