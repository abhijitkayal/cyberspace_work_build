...
export default function QuotationPage() {
  ...
  // Use fetch + credentials so the session cookie is sent,
  // then trigger a browser download from the blob response.
  const handleDownload = async (q: Quotation) => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `${(q.title || "quotation").replace(/[^a-zA-Z0-9._-]+/g, "-")}.pdf`;
          ...
          {data.map((q: Quotation) => (
            ...
            >
              ...
              <div className="flex items-center justify-between gap-3">
                ...
                {/* Show Download button whenever there is a fileUrl */}
                {q.fileUrl ? (
                  <button
                  ...
                  <span className="text-xs text-gray-400 dark:text-gray-600 italic">No file</span>
                )}
              </div>
            </div>
          ))}
