...
import EmployeeDashboardPanel from "@/components/projects/EmployeeDashboardPanel";
...
export default async function EmployeeDashboardPage() {
  ...
  // serialize similar to client page to avoid passing non-plain objects
  ...
  const projectsForEmployee = projects.map(serializeProject);

  return (
    <div className="space-y-6">
      <EmployeeDashboardPanel projects={projectsForEmployee} sessionUserId={session.user.id} />
    </div>
  );
}
